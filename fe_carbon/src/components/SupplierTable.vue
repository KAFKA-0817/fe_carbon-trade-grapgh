    ...
    <el-container>
      <el-header style="margin-top: 50px">
        <el-form :inline="true" class="demo-form-inline">
          <el-form-item label="Supplier Name">
            <el-input placeholder="输入采购部门结点名" />
          </el-form-item>
          <el-form-item label="Supplier Coordinate">
            <el-input placeholder="输入采购部门结点坐标" />
          </el-form-item>
          <el-form-item>
      ...
      <el-main>
        <el-table :data="tableData" stripe style="width: 100%">
          ...
          <el-table-column prop="capacity" label="采购能力" width="180" />
          <el-table-column prop="cpp" label="单位采购成本" />
  ...
  <el-dialog v-model="addDialogVisible" title="Shipping address">
    <el-form :model="addForm">
      ...
      <el-form-item label="cpp" :label-width="formLabelWidth">
        <el-input v-model="addForm.cpp" autocomplete="off" size="small" />
      </el-form-item>
...
import { getAllSuppliers } from "../service/api.js";
...
export default {
  ...
  setup() {
    ...
    onMounted(() => {
      getAllSuppliers({ current: "1", size: size }).then((res) => {
    ...
    const reloadTable = () => {
      getAllSuppliers({ current: current.value, size: size }).then((res) => {
        tableData.value = res.data.data.records;
        current.value = res.data.data.current;
        pages.value = res.data.data.pages;
      });

    ...
    <el-container>
      ...
      <el-main>
        <el-table :data="tableData" stripe style="width: 100%">
          ...
          <el-table-column prop="capacity" label="配送能力" width="180" />
...
import { getAllDistributors } from "../service/api.js";
...
export default {
  ...
  setup() {
    ...
    onMounted(() => {
      getAllDistributors({ current: "1", size: size }).then((res) => {
    ...
    const reloadTable = () => {
      getAllDistributors({ current: current.value, size: size }).then((res) => {
        tableData.value = res.data.data.records;
        current.value = res.data.data.current;
        pages.value = res.data.data.pages;
      });
